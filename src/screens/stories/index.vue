...
import ModalOption from '../../components/share/ModalOption.vue'
import HeaderImage from "../../components/share/HeaderImage.vue"
import * as LocalStoryData from '../../localstore/story.js'

export default {
  props: ['navigation'],
  components: { ShareApp, Loading, HeaderImage, Story , ModalOption },
  data() {
    return {
      currentStory: null,
      book: null,
  ...
  created(){
    ...
    this.stories = this.navigation.getParam('stories')
  ...
  async mounted() {
    this.axios
     .put(`/api/users/update_view_mode`, {view_mode: "tap"})
     .then((res) => {})
     .catch((error) => console.log("error update_view_mode", error) )

...
AppNavigation.router.getStateForAction = (action, state) => {
  if (
    ...
    let newRoutes = state.routes.filter(
      (r) => r.routeName !== "Story" && r.routeName !== "BookDetail"
    );
    ...
    let newRoutes = state.routes.filter(
      (r) =>
        r.routeName !== "Story" ||
        (r.routeName === "Story" && r === state.routes[state.index])
    );
    let newIndex = newRoutes.length - 1;
...
export default {
  ...
  methods: {
    ...
    saveTrackingPageView(route, pre_route) {
      switch (route) {
        case "Home":
          if (route != pre_route) this.incrementPageViews("pv_top");
          break;
        case "Tags":
          if (route != pre_route) this.incrementPageViews("pv_tags");
          break;
        case "TagDetail":
          if (route != pre_route) this.incrementPageViews("pv_tag_detail");
          break;
        case "BookDetail":
          if (route != pre_route) {

...
const getters = {
  ...
  is_scroll_mode: state => state.is_scroll_mode
}
...
const actions = {
  ...
  async androidPaySub(subscription, commit){
    try {
      ...
      await InAppBilling.open()
      commit('updateLoading', false)
      console.log("vao day chua", subscription)
      const details = await InAppBilling.subscribe(subscription.key_test)
      console.log("vao day chua11", subscription)
      ...
      console.log("vao day chua2", error)
  ...
  async cancelSubscription(commit){
    if(Platform.OS=="ios"){
      ...
      await axios.delete('/api/remove_user_subscription').then(() => {
        writeLog( axios,  { event: 'CANCEL_SUBSCRIPTION', value: "cancle subscript cause don't error purchase", log_type: 'info' })          
        commit('updateCurrentSubscription', null)
      }).catch(error=>{
        console.log(error, "Error cancle android sub")
      })  
    }
  },
  ...
  update_is_scroll_mode({ commit }, status) {
    commit('update_is_scroll_mode', status)
  }
}
...
const mutations = {
  ...
  update_is_scroll_mode(state, status) {
    state.is_scroll_mode = status
  }
}

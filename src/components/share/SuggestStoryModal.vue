...
export default {
  ...
  methods: {
    ...
    setPopupData(data) {
      // console.log(data, "arrr popup")
          let popupData = data;
          if (popupData) {
            this.popupArr = popupData;
    ...
    handleNextPopup() {
      this.indexPopup++;
      console.log(this.indexPopup, this.popupArr.length, this.popupArr[this.indexPopup])
      if(this.indexPopup<this.popupArr.length&&this.popupArr[this.indexPopup]){
        Image.getSize(this.popupArr[this.indexPopup].img_url.url, (width, height) => {
              let ratio = width / height;
              if (ratio > 1) {
                this.imageHorizontal = true;
              }
              this.ratioPopup = ratio;
              this.popupState = this.popupArr[this.indexPopup];              
              this.showModal();
            });
            if(this.indexPopup==this.popupArr.length-1){
              return 
            }            
      }else{
        this.hideModal()      
...
<style>
.background-modal {
  background-color: rgba(18, 2, 2, 0.5);
}
</style>

<template>
  <view class="container">
    ...
    <lock-old-device-modal v-if="isExitApp" :clearData="clearData" />
...
export default {
  ...
  mounted: async function () {
    ...
    this.axios.post(`/api/logs/debug_crash`, {
      event: "Splash",
      value: uuid,
      log_type: "info",
  ...
  methods: {
    ...
    async checkATT() {
      try {
        ...
        if (
          ...
          if (
            requestTrackingStatus === "authorized" ||
            requestTrackingStatus === "unavailable"
          ) {
            // enable tracking features
          }
        }
    ...
    initAppsFlyer() {
      ...
      if (Platform.OS === "ios") {
        options.appId = Config.APPSFLYER_ID_TEST_IOS;
      }
      ...
      appsFlyer.onInstallConversionData((data) => {
        ...
        if (
          ...
          this.axios
            .post(`/api/reports/update_page_view/`, {
      ...
      appsFlyer.initSdk(
        ...
        (error) => {
          console.error("initsdk appsflyer error: ", error);
        }
      );
    },
    ...
    async onFinish() {
      ...
      SplashScreen.hide();
      await this.sendNotifyTokenToServer();
    ...
    async sendNotifyTokenToServer() {
      const enable = await NotificationService.checkNotificationPermission();
      if (!enable) return;
      const fcmToken = await NotificationService.getNotificationFcmToken();
      if (!fcmToken) return;

      this.axios
        .post(`/api/devices/update_device_token/`, {
